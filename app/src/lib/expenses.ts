...
import { ApiError, api, type Attachment, type CreateExpenseInput, type Expense } from './api';

export function createExpense(input: CreateExpenseInput): Promise<Expense> {
	return api<Expense>('/expenses', { method: 'POST', body: input });
}

export function updateExpense(id: string, input: CreateExpenseInput): Promise<Expense> {
 ...
 */
export const ATTACHMENT_ACCEPT = 'image/jpeg,image/png,image/heic,image/heif,application/pdf';
...
async function compressImage(file: File): Promise<Blob> {
	...
	try {
		...
		for (const q of ATTACHMENT_IMAGE_QUALITY_STEPS) {
			...
			// input AND the input is already JPEG.
			if (file.type === 'image/jpeg' && blob.size >= file.size) continue;
			if (blob.size <= ATTACHMENT_IMAGE_TARGET_BYTES || q === ATTACHMENT_IMAGE_QUALITY_STEPS.at(-1)) {
				return blob;
			}
		}
		return file;
 ...
 */
export async function prepareForUpload(file: File): Promise<PreparedAttachment> {
	...
	if (isImage) {
		...
		throw new ApiError(400, 'UNSUPPORTED_TYPE', `Type non supporté: ${file.type || 'inconnu'}`);
	}
...
function putToGCSWithProgress(
	...
	return new Promise<void>((resolve, reject) => {
		...
		xhr.onload = () => {
			...
			else reject(new ApiError(xhr.status, 'GCS_UPLOAD_FAILED', `upload failed (${xhr.status})`));
		};
		xhr.onerror = () => reject(new ApiError(0, 'GCS_UPLOAD_FAILED', 'network error during upload'));
		xhr.onabort = () => reject(new ApiError(0, 'UPLOAD_ABORTED', 'upload was cancelled'));
...
export async function materializeRecurring(): Promise<MaterializeSummary | null> {
	...
	return api<MaterializeSummary>('/expenses/materialize-recurring', { method: 'POST' });
}

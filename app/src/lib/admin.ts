...
export async function adminApi<T>(path: string, opts: AdminOptions = {}): Promise<T> {
...
export function createFoyer(input: CreateFoyerInput): Promise<CreateFoyerResponse> {
	return adminApi<CreateFoyerResponse>('/admin/foyers', { method: 'POST', body: input });
}
...
export function addFoyerMember(foyerId: string, member: MemberInput): Promise<AddMemberResponse> {
	return adminApi<AddMemberResponse>(`/admin/foyers/${encodeURIComponent(foyerId)}/members`, {
		method: 'POST',
		body: member
	});
}

export function updateFoyerParts(foyerId: string, parts: number): Promise<void> {

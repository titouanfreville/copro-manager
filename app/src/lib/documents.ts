...
export function updateDocument(
	id: string,
	input: { title: string; description?: string; category_id: string; group?: string }
): Promise<Document> {
...
export function deleteDocument(id: string): Promise<void> {
	return api<void>(`/documents/${encodeURIComponent(id)}`, { method: 'DELETE' });
}
...
async function putToGCS(
	...
	if (typeof XMLHttpRequest === 'undefined' || !opts.onProgress) {
		...
		if (!res.ok) {
			throw new ApiError(res.status, 'GCS_UPLOAD_FAILED', `upload failed (${res.status})`);
		}
		return;
	}
